...
class RerenderablePage extends MultipleCodeExamples3Page() {

  override def pageTitle: String = "FastScala Rerenderable"

  import com.fastscala.components.bootstrap5.helpers.BSHelpers.*

  override def pageLead(implicit fsc: FSContext): NodeSeq = p.apply(
    """Use the rerenderable when you want to have a part of the page that can be rerendered."""
  )

  override def renderAllCodeSamples()(implicit fsc: FSContext): Unit = {

    renderCodeSampleAndAutoClosePreviousOne("Rerenderable from outside") {
    ...
    renderCodeSampleAndAutoClosePreviousOne("Rerenderable with arguments") {
